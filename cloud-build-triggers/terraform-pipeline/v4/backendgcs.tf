...
resource "google_storage_bucket" "backend_state_bucket" {
  ...
  name      = "${var.gcs_bucket_unique_id}-terraform-backends-${random_string.terraform_backend_random_string.result}"
...
/******************************************
  Random String
 *****************************************/
resource "random_string" "terraform_backend_random_string" {
  length    = 10
  lower     = true
  upper     = false
  special   = false
